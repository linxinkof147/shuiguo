		...
		<view class="popup-bg" :style="getHeight">
			<view class="popup-content" :class="{'popup-content-show' : popup_show}">
				<view class="update-wrap">
					<image src="./images/img.png" style="width: 120rpx;height: 100rpx;margin-top: 160rpx;margin-left: 240rpx;" class="top-img"></image>
					<view class="content">
						<text class="title" style="color: #ff6a6c;">发现新版本V{{update_info.version}}</text>
						<!-- 升级描述 -->
						<view class="title-sub" v-html="update_info.note"></view>
						<!-- 升级按钮 -->
						<button class="btn" v-if="downstatus < 1" @click="nowUpdate()" style="background-color: #ff6a6c;">立即升级</button>
						<!-- 下载进度 -->
	...
	export default {
		...
		data() {
			return {
				...
				newversionNo:'1.2.0',
				downLoadUrl:'https://bsppub.oss-cn-shanghai.aliyuncs.com/VKCEYUGU-aa5779fe-cd84-4a6f-85d0-3e72447428bf/f98a8d86-3ca0-4c93-a87a-dfb09190345c.apk'
			};
		},
		created() {
			vm = this;
		...
		methods: {
			...
			async newVersion(){
				...
				this.downLoadUrl = res.body.versionDes
				this.newversionNo = res.body.versionNo
			},	
			// 获取线上版本信息
			getUpdateInfo() {
				//向后台发起请求，获取最新版本号
				uni.request({
					...
					success: (res) => {
						...
						let data = [{
							os: 'android',
							version: this.newversionNo, //最新版本
							note: 'Android版本<br>请更新后使用,修改支付白屏',
							download_url: this.downLoadUrl, //下载地址 (微信测试)
						}, {
							os: 'ios',
							version: '1.0.1', //最新版本
							note: '我是IOS升级说明</br>升级说明', //下载地址
							download_url: 'https://bsppub.oss-cn-shanghai.aliyuncs.com/VKCEYUGU-aa5779fe-cd84-4a6f-85d0-3e72447428bf/f98a8d86-3ca0-4c93-a87a-dfb09190345c.apk', //微信测试
						}];
